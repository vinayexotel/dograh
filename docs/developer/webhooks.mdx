...
ended. See [Call Dispositions](./call-dispositions) to configure custom business
outcomes for each workflow and understand the fallback behavior.

<Note>
Dograh always includes a top-level `call_disposition` field in the delivered
...
```json
{
  ...
  "mapped_disposition": "{{gathered_context.mapped_call_disposition}}",
