...
Traces sent to your own Langfuse project are private by default — opening a trace link requires access to that project. **Make traces publicly viewable** in **Platform Settings** turns that off for your project only.
...
`LANGFUSE_TRACES_PUBLIC` is the equivalent switch for the Langfuse project configured by environment variables. It has no effect on organizations that have connected their own Langfuse project — those follow the setting above.
